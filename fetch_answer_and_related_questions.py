...
class fetch_answer_and_related_questions:
	...
	def fetch_related_questions_and_links(self):
		...
		df_questions_with_corresponding_links.to_excel(str(self.topic_dir)+"/"+str(self.topic)+"_links_related.xlsx")
		driver.close()
		return 
	...
	def run(self):
		...
		df_related = pd.read_excel(str(self.topic_dir)+"/"+str(self.topic)+"_links_related.xlsx",sheet_name="Sheet1")
